...
export class StudentsDataDBService {
  ...
  getStudentsData(){
    return Observable.create((subscriber:Subscriber<Student[]>)=>subscriber.next(this.students));
  }

  getStudent(id:number){
    let student = this.students.find(student=> student.id === +id);
    return Observable.create((subscriber:Subscriber<Student>)=>subscriber.next(student));
  }
